...
# Validação de acesso baseada exclusivamente na lista de permissões
current_user = st.session_state.get("user")
user_views = current_user.get("views", []) if current_user else []

if not current_user or ("views/98.Admin_Users.py" not in user_views and current_user.get("username") != "admin"):
    st.error("⛔ Acesso não autorizado. Você não possui permissão para acessar a Gestão de Usuários.")
    st.stop()
...
def dialog_editar_usuario(username: str):
    ...
    with c_btn1:
        if st.button("Salvar Alterações", type="primary", use_container_width=True):
            ...
            if success:
                st.success(msg)
                # Se o usuário editado for o usuário atual da sessão, atualiza os dados na sessão
    ...
    for u in users:
        ...
        with cols[5]:
            ...
            with act_c3:
                ...
                is_disabled = (u.username == "admin") or (u.username == current_user.get("username"))
                if st.button("🗑️", key=f"btn_del_{u.id}", help=f"Excluir @{u.username}", disabled=is_disabled):
                    dialog_confirmar_exclusao(u.username)
                    
        st.markdown("<hr style='margin: 4px 0; border-color: #33333333;'>", unsafe_allow_html=True)
